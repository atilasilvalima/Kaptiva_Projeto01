...
if ((empty($DataInicio)) AND (empty($DataTermino))) {
    ...
    // SQL Total Horas Gastas
    $SQL_TotalHorasGastas = "
                    SELECT
                        (SUM({edwreports_activity_log}.timespent)/3600) AS HORAS
                        FROM
                            {edwreports_activity_log}
                        WHERE
                            {edwreports_activity_log}.timespent > 0 AND
                            {edwreports_activity_log}.timestart BETWEEN ? AND ?
                    ";
    $RES_TotalHorasGastas = $DB->count_records_sql($SQL_TotalHorasGastas,[$DataInicio,$DataTermino]);
    ?>
                                ...
                                foreach ($RES_ListaCategoriasHoras as $ListaCategoriasHoras) {
                                    ...
                                    $SQL_TotalHorasCategoria = "
                                                        ...
                                                        INNER JOIN
                                                            {course}
                                                        ON 
                                                            {edwreports_activity_log}.course = {course}.id
                                                        INNER JOIN
                                                            {course_categories}
                                                        ON 
                                                            {course}.category = {course_categories}.id
                                                        WHERE 
                                                            ...
                                                            {edwreports_activity_log}.timestart BETWEEN ? AND ?
                                                    ";
                                    $TotalHorasCategoria = $DB->count_records_sql($SQL_TotalHorasCategoria,[$DataInicio,$DataTermino]);

                                    echo "<td>" .$TotalHorasCategoria. "</td>";

                                    // % de horas
                                    $PorcHorasCategoria = (100*$TotalHorasCategoria)/$RES_TotalHorasGastas;
                                    $PorcHorasCategoria = number_format($PorcHorasCategoria,2,",",".");
                                    echo "<td>".$PorcHorasCategoria."%</td>";
                                }
                                echo "</tr>";
                                ?>
                                </tbody>
